...
Element announcements =  XMLUtil._Proxy.getFirstChildByTagName(course, XMLBuilder._Static._Proxy.$instance.get$TAG_COURSEANNOUNCEMENTS());
if (announcements != null) { 
boolean inStudentView =  URL.equals(AccessController.COURSE_URL);
String divID = inStudentView ? "id=\"topblock\"" : "";
int numTotalAnnounce = Integer.parseInt(announcements.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_NUMTOTALANNOUNCE()));
int numRecentAnnounce = Integer.parseInt(announcements.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_NUMRECENTANNOUNCE()));
boolean isadmin = false, iscategory = false;
if (!p.isInStaffAsBlankMode()) {
  isadmin= course.hasAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ISADMIN()) && !inStudentView;
...
if (length == 0) { %>
  ...
  for (int i= 0; i != list.getLength(); i++) { 
    ...
    String poster= item.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_POSTER()); 
    String txt = item.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_TEXT());
    String editInfo = item.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_OLDTEXT());
    String announceid = item.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ID());
    NodeList history = item.getElementsByTagName(XMLBuilder._Static._Proxy.$instance.get$TAG_ANNOUNCEMENTHISTORY()).item(0).getChildNodes(); %>
            <tr>
              <td>
                <p style="margin-left: 3em; font-size: smaller"><i><%= item.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_DATE()) %></i><%
...
NodeList hiddens = root.getElementsByTagName(XMLBuilder._Static._Proxy.$instance.get$TAG_HIDDENANNOUNCE());
if (hiddens.getLength() > 0 && (isadmin || iscategory)) { %>
    ...
    <table width="100%" cellpadding="2" cellspacing="0" border="0">
      <% for (int i=0; i < hiddens.getLength(); i++) { 
          ...
          <tr><td>Posted by <%= xHiddenAnnounce.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_POSTER()) %> on <%= xHiddenAnnounce.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_DATE()) %>&nbsp;
          <a href="?<%= AccessController.P_ACTION + "=" + AccessController.ACT_RESTOREANNOUNCE + "&amp;" + AccessController.P_ID + "=" + xHiddenAnnounce.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ID()) %>">Restore</a></td>
          </tr><tr><td><p style="margin-left: 20px">
          <%= xHiddenAnnounce.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_TEXT()) %></p></td></tr>
      <% } %>
    </table></div>
<%
}

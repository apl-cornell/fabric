...
public class HandshakeUnauthenticated<Node extends RemoteNode<Node>>
  ...
  public ShakenSocket<Node> initiate(Node remoteNode, SocketChannel s)
    ...
    out.flush();
    return new ShakenSocket<>(remoteNode.name,
        new RemoteIdentity<>(remoteNode, null), s);
  }
  ...
  public ShakenSocket<RemoteWorker> receive(SocketChannel s)
    ...
    return new ShakenSocket<>(name, new RemoteIdentity<>(remoteWorker, null),
        s);
  }

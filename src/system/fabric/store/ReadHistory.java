...
/**
 * This class is responsible for keeping a log of the recent reads, for checking
 * commit conflicts. Each read has a time associated with it, and the log only
...
public class ReadHistory {
   ...
   * 
   * @param length
   *          the duration, in milliseconds, of entries in the history
   * @param beginning
   *          a time such that no reads have happened since that time (typically
   *          now)
   */
  ...
  private synchronized void clean(long now) {
    ...
    while (!history.isEmpty()) {
      ...
      while (i.hasNext()) {
        ...
        if (time != null && time < cutoff) reads.remove(oid);
      }
  ...
  public synchronized boolean check(long oid, long time) {
    if (time < this.currentSince) return false;
   ...
   * <code>length</code> milliseconds and removes any entries older than now -
   * length, updating <code>currentSince</code> accordingly.
   */

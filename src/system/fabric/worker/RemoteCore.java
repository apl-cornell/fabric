 ...
 * <code>Worker.getCore()</code> interface. For each remote core, there should
...
public class RemoteCore extends RemoteNode implements Core {
  ...
  /*
   * (non-Javadoc)
   * @see fabric.worker.Core#readObjectNoDissem(long)
   */
  ...
  private final Object._Impl readObject(boolean useDissem, long onum)
    ...
    if (ONumConstants.isGlobalConstant(onum))
      return Worker.instance.localCore.readObject(onum);
  ...
  public java.lang.Object readResolve() {
    return Worker.getWorker().getCore(name);
  }
  ...
  /*
   * (non-Javadoc)
   * @see fabric.worker.Core#abortTransaction(long)
   */
  ...
  /*
   * (non-Javadoc)
   * @see fabric.worker.Core#commitTransaction(int)
   */
  ...
  /**
   * Updates the worker's cache of objects that originate from this core. If an
  ...
  /**
   * Clears the worker's cache for this core. To be used for (performance)
   * testing only.
   * 

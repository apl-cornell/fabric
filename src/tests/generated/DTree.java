...
public class DTree extends DObject {
  ...
  private void set_root(DTreeNode new_root) {
    if (new_root == null)
      root_ref = null;
    else root_ref = new_root.this_ref;
  }
  ...
  private DTreeNode insert_helper(DTreeNode current, DInt value) {
     ...
     * insert_helper( current.right, value ); else current.left = insert_helper(
     * current.left, value ); return current;
     */
    if (current == null)
    ...
    else current.set_left(insert_helper(current.get_left(), value));
    return current;
  }
  ...
  public void insert_iterative(DInt value) {
     ...
     * (current.value.value < value.value) { if (current.right == null) {
     * current.right = new TreeNode@node_location( value, null, null ); break; }
     * else current = current.right; } else { if (current.left == null ) {
     * current.left = new TreeNode@node_location( value, null, null ); break; }
  ...
  public DInt lookup(DInt value) {
     ...
     * (current.value.value < value.value) current = current.left; else current =
     * current.right; } return null;
     */
    DTreeNode current = get_root();
    while (current != null) {
      if (current.get_value().get_value() == value.get_value())

/*
 * @(#)AbstractList.java 1.52 06/06/16 Copyright 2006 Sun Microsystems, Inc. All
 ...
 * programmer must additionally override the
...
public abstract class AbstractList extends AbstractCollection implements List {
   ...
   * <p>
   * Note that this implementation throws an
   * {@code UnsupportedOperationException} unless {@code remove(int index)} or
   ...
   * {@code remove(int)}, {@code addAll(int, Collection)} and
   * {@code removeRange(int, int)} methods all delegate to the corresponding
   ...
   * iterator on the backing list, which is created with the corresponding
   * method on the backing list. The {@code iterator} method merely returns
   ...
   * lists are <i>equal</i>. (Two elements {@code e1} and {@code e2} are
   * <i>equal</i> if {@code (e1==null ? e2==null : e1.equals(e2))}.) In other
   * words, two lists are defined to be equal if they contain the same elements
   * in the same order.
   * <p>
   ...
   * lists, comparing corresponding pairs of elements. If any comparison returns
   * {@code false}, this method returns {@code false}. If either iterator runs
   * out of elements before the other it returns {@code false} (as the lists are
   * of unequal length); otherwise it returns {@code true} when the iterations
   * complete.
   * 
   * @param o
   ...
   * list implementation can <i>substantially</i> improve the performance of
   * the {@code clear} operation on this list and its subLists.
   * <p>
   * This implementation gets a list iterator positioned before
   * {@code fromIndex}, and repeatedly calls {@code ListIterator.next} followed
   * by {@code ListIterator.remove} until the entire range has been removed.
   ...
   * to provide fail-fast iterators (and list iterators), then it merely has to

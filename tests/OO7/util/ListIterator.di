/*
 * @(#)ListIterator.java 1.28 06/04/21 Copyright 2006 Sun Microsystems, Inc. All
 ...
 * current position in the list. A <TT>ListIterator</TT> has no current
 * element; its <I>cursor position</I> always lies between the element that
 * would be returned by a call to <TT>previous()</TT> and the element that
 * would be returned by a call to <TT>next()</TT>. An iterator for a list of
 * length <tt>n</tt> has <tt>n+1</tt> possible cursor positions, as
 * illustrated by the carets (<tt>^</tt>) below:
 * 
 * <PRE>
 * 
...
public interface ListIterator extends Iterator {
   ...
   * to iterate through the list, or intermixed with calls to <tt>previous</tt>
   * to go back and forth. (Note that alternating calls to <tt>next</tt> and
   * <tt>previous</tt> will return the same element repeatedly.)
   * 
   ...
   * or <tt>previous</tt> (optional operation). This call can only be made
   * once per call to <tt>next</tt> or <tt>previous</tt>. It can be made
   * only if <tt>ListIterator.add</tt> has not been called after the last call
   ...
   *              added to this list.
   * @exception IllegalStateException
   *              if neither <tt>next</tt> nor <tt>previous</tt> have been
   *              called, or <tt>remove</tt> or <tt>add</tt> have been
   *              called after the last call to <tt>next</tt> or
   *              <tt>previous</tt>.
   */

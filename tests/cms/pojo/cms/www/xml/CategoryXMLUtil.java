...
public class CategoryXMLUtil
{
	...
	public final String REQ_ID_SEP = "_";
	 ...
	 */
	public String createContentIDsString(long contentID, long rowID, long colID)
	{
	 ...
	 * (some subset of the input IDs, with delimiters)
	 */
	public String createFileIDsString(long fileID, long contentID, long rowID, long colID, int fileIndex)
	{
	 ...
	 */
	public NodeList getListByTagName(Element parent, String tag){
	...
	public NodeList getVisibleColumnList(Element category){
	 ...
	 * @return
	 */
	public NodeList getColumnList(Element category)
	{
		Element columnList = XMLUtil.getFirstChildByTagName(category, XMLBuilder.TAG_COLUMNS);
		return XMLUtil.getChildrenByTagName(columnList, XMLBuilder.TAG_COLUMN);
	}
	
	public NodeList getContentList(Element row){
	...
	public String printURL(Element content)
	{
	...
	public String printText(Element content)
	{
		if(Long.parseLong(content.getAttribute(XMLBuilder.A_ID)) == -1 //id -1 means no actual content in db
	...
	public String printNumber(Element content)
	{
		if(Long.parseLong(content.getAttribute(XMLBuilder.A_ID)) == -1) //id -1 means no actual content in db
			return printBlank();
		return content.getAttribute(XMLBuilder.A_DATA);
	...
	public String printDate(Element content)
	{
		if(Long.parseLong(content.getAttribute(XMLBuilder.A_ID)) == -1) //id -1 means no actual content in db
			return printBlank();
	...
	public String printFile(Element content)
	{
		if(Long.parseLong(content.getAttribute(XMLBuilder.A_ID)) == -1) //id -1 means no actual content in db
			return printBlank();
		String result = "";

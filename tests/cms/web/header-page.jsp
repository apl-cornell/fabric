...
<ul>
<%
Node l = root.getElementsByTagName(XMLBuilder.$Static.TAG_STUDENTCOURSES).item(0);
int length= 0;
NodeList myCourses = l.getChildNodes();
length= myCourses.getLength();
String link=null;
String courseName=null;
boolean isAdminForAnyCourse= (root.getElementsByTagName(XMLBuilder.$Static.TAG_STAFFCOURSES).item(0).getChildNodes().getLength()!=0);
...
for (int i = 0; i < length; i++) {
  ...
  link= "?"+ AccessController.P_ACTION + "=" + AccessController.ACT_COURSE + "&amp;" + AccessController.P_COURSEID + "=" + course.getAttribute(XMLBuilder.$Static.A_COURSEID) + "&amp;" + AccessController.P_RESET + "=1";
  courseName=course.getAttribute(XMLBuilder.$Static.A_DISPLAYEDCODE);
%>   
...
<%
/* STAFF */
l= root.getElementsByTagName(XMLBuilder.$Static.TAG_STAFFCOURSES).item(0);
...
for (int i= 0; i != length; i++) { 
  Element course= (Element) myCourses.item(i); 
  link= "?"+ AccessController.P_ACTION + "=" + AccessController.ACT_COURSEADMIN  + "&amp;" + AccessController.P_COURSEID + "=" + course.getAttribute(XMLBuilder.$Static.A_COURSEID);
  courseName=course.getAttribute(XMLBuilder.$Static.A_DISPLAYEDCODE);

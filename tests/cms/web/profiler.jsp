...
NodeList actions = root.getElementsByTagName(XMLBuilder.$Static.TAG_REQUESTPROFILE);
%>
...
<% for(int j=0; j < actions.getLength(); j++) {
	...
	<table cellpadding="0" cellspacing="1" border="1" width="90%">
		...
		<% for(int i=0; i < calls.getLength(); i++) { 
			...
			String type = method.getAttribute(XMLBuilder.$Static.A_TYPE);
			boolean IN = type != null && type.equals("IN"); %>
			<tr class="<%= type %>">
				<td>&nbsp;<%= method.getAttribute(XMLBuilder.$Static.A_METHODNAME)%></td>
				<td>&nbsp;<%= type %></td>
				<td>&nbsp;<%= method.getAttribute(XMLBuilder.$Static.A_CONTEXT)%></td>
				<td>&nbsp;<%= method.getAttribute(XMLBuilder.$Static.A_TIMEPERIOD)%></td>
				<td>&nbsp;<%= method.getAttribute(XMLBuilder.$Static.A_PERCENT)%></td>
				<td>&nbsp;<%= method.getAttribute(XMLBuilder.$Static.A_TIME)%></td>
			</tr>
		<% } %>
	</table><br><br>
<% } %>

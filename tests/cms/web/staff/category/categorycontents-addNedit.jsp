<%@ page language="java" import="org.w3c.dom.*, edu.cornell.csuglab.cms.www.*, edu.cornell.csuglab.cms.www.xml.*,edu.cornell.csuglab.cms.util.category.*"%>
	...
	<table id="course_wrapper_table" summary="course wrapper" cellpadding="0" cellspacing="0" border="0" width="100%">
		<tr>
			...
			<td id="course_page_container" >
				<form action="?<%= AccessController.P_ACTION %>=<%= AccessController.ACT_SETADDNEDITCONTENTS %>&amp;<%= AccessController.P_COURSEID %>=<%= course.getAttribute(XMLBuilder.A_COURSEID) %>&amp;<%= AccessController.P_CATID %>=<%= category.getAttribute(XMLBuilder.A_ID) %>" onSubmit="return validateAllFormattedTextboxes();" method="post" enctype="multipart/form-data">
					<div id="course_page">
...
if(category.hasAttribute(XMLBuilder.A_ISANNOUNCEMENTS)) /* announcements get nice big boxes */
{
	rowSize = "3";
	colSize = "30";
}%>
...
<%	long[] colIDArr = new long[columnList.getLength()];
  	for(int column=0; column < columnList.getLength(); column ++)
  	{
		...
		<th id="<%= colIDArr[column] %>" title="<%= ctgCol.getAttribute(XMLBuilder.A_TYPE) %>" a_colId="<%= ctgCol.getAttribute(XMLBuilder.A_ID) %>" a_coltype="<%= ctgCol.getAttribute(XMLBuilder.A_TYPE) %>" nowrap>
<%		if(ctgCol.getAttribute(XMLBuilder.A_HIDDEN).equals("true"))
		{%>
			<font color="#8888aa">
				<%= ctgCol.getAttribute(XMLBuilder.A_NAME) + " [HIDDEN]" %>
			</font>
<%		}
		else
		{%>
			<font>
				<%= ctgCol.getAttribute(XMLBuilder.A_NAME) %>
			</font>
<%		}%>
		</th><%
	} %>
...
<%   	String rowIDString = "" + ctgRowID;
   	...
   	for(int column=0; column<contentList.getLength(); column++)/*Each Column Of A Row*/
   	{
		...
		<td id="<%= "cell" + ctntIDString %>" align="left" numfiles="<%= fileList.getLength() %>">
<%					if(fileList.getLength() > 0) /* display table of visible files (don't display hidden) */
					{%>
			<div class="replace">
			<span id="<%= fileType + SEP + ctntIDString + "head" %>">
				<a href="#" onClick="show('<%= fileType + SEP + ctntIDString %>', '<%= sectionLabel %> &raquo;', '&laquo; <%= sectionLabel %>'); return false;"><%= sectionLabel %> &raquo;</a>
			</span>
			<table class="replace" id="<%= fileType + SEP + ctntIDString %>" cellpadding="0" cellspacing="0" style="display:none">
				<tr>
					<th>Shown As</th>
					<th>Filename</th>
					<th>Remove</th>
...
<%						/* display table of visible (nonhidden) files */
				...
				<tr class="<%= count % 2 == 0 ? "row_even" : "row_odd" %>">
					<td align="left">
						<input type="text" size="15" name="<%= AccessController.P_CONTENT_FILELABEL + SEP + fileIDString %>" value="<%= file.getAttribute(XMLBuilder.A_LINKNAME) %>">
					</td>
					<td align="left">
						<%= file.getAttribute(XMLBuilder.A_FILENAME).equals("") ? "(no file)" : file.getAttribute(XMLBuilder.A_FILENAME) %>
					</td>
					<td class="remove">
						<input type="checkbox" name="<%= AccessController.P_REMOVEFILE + SEP + fileIDString %>">
					</td>
				</tr>
<%						}
...
<%					}
				...
				{%>
		<td id="<%= "cell" + ctntIDString %>" align="left" numfiles="0">
<%				}%>
			<div class="replace">
...
<%				numOfCal++;	
			...
			{%>
		<td align="center" id="<%= "cell" + ctntIDString %>">
			<textarea rows="<%= rowSize %>" cols="<%= colSize %>" physical name="<%= AccessController.P_CONTENT_TEXT + SEP + ctntIDString %>"><%=content.getAttribute(XMLBuilder.A_DATA)%></textarea>
		</td>
<%			}
			else if(datatype.equalsIgnoreCase(CtgUtil.CTNT_NUMBER))
			{%>
		<td align="center" id="<%= "cell" + ctntIDString %>">
			<input type="text" size="6" id="editnumber_<%= ctntIDString %>" name="<%= AccessController.P_CONTENT_NUMBER + SEP + ctntIDString %>" value="<%=content.getAttribute(XMLBuilder.A_DATA)%>">
		...
		<td align="center" id="<%= "cell" + ctntIDString %>">
			<table border="0">
				<tr class="no_borders">
					...
					<td align="left">
						<input type="text" size="20" name="<%= AccessController.P_CONTENT_URLADDRESS + SEP + ctntIDString %>" value="<%=content.getAttribute(XMLBuilder.A_URL)%>">
					</td>
				</tr>
				<tr>
					...
					<td border="0" align="left">
						<input type="text" size="20" name="<%= AccessController.P_CONTENT_URLLABEL + SEP + ctntIDString %>" value="<%=content.getAttribute(XMLBuilder.A_LINKNAME)%>">
					</td>
				</tr>
			</table>
      </td>
<%  		}
		...
		<td align="center">
		  <input type="checkbox" name="<%= AccessController.P_REMOVEROW + SEP + rowIDString %>">
	 	</td>
	  </tr>
<% } /*End Each Row */ %>
    </table><%
...
if(columnList.getLength() > 0)
{
	...
	<div class="replace">
	    <span id="removedcontentshead">
	      <a href="#" onClick="show('removedcontents', 'Removed contents &raquo;', '&laquo; Removed contents'); return false;">Removed contents &raquo;</a>
	    </span>
...
<% 	for(int row=0; row<rowListHidden.getLength(); row++) /* display hidden rows */
		{
...
<%	     	}%>
				<td>
					<input type="checkbox" name="<%= AccessController.P_RESTOREROW + SEP + rowIDString %>">
				</td>
	 		</tr>
<%		} /* end hidden rows */%>
	    </table>

...
Element assign= XMLUtil._Proxy.getFirstChildByTagName(root, XMLBuilder._Static.TAG_ASSIGNMENT); 
Element groupsnode= XMLUtil._Proxy.getFirstChildByTagName(root, XMLBuilder._Static.TAG_GROUPS);
Element probsnode= XMLUtil._Proxy.getFirstChildByTagName(root, XMLBuilder._Static.TAG_SUBPROBS);
...
String assignid= assign.getAttribute(XMLBuilder._Static.A_ID);
...
int assignType = Integer.parseInt(assign.getAttribute(XMLBuilder._Static.A_ASSIGNTYPE));
boolean isAssignment = (assignType == Assignment._Static.ASSIGNMENT);
boolean hasSection = course.hasAttribute(XMLBuilder._Static.A_HASSECTION);
...
boolean isGroups = course.hasAttribute(XMLBuilder._Static.A_ISGROUPS);
boolean assignedGraders = assign.hasAttribute(XMLBuilder._Static.A_ASSIGNEDGRADERS);
int assignedCount = 0;
%>
...
<%	for (int i= 0; i != numprobs; i++) {
    ...
    <th nowrap style="display: none" align="center" class="scores" id="assigned_<%= assignedCount++ %>" name="assigned"><%= prob.getAttribute(XMLBuilder._Static.A_NAME) %></th><%
	}%>
<%--- ASSIGNEDTO HEADER CELLS END -----%>
<%--- SUBPROBLEM GRADE HEADER CELLS BEGIN -----%>
<% 	for(int j= 0; j < numprobs; j++) {
		Element prob= (Element)probs.item(j);
		String text = (isAssignment) ? prob.getAttribute(XMLBuilder._Static.A_NAME) : prob.getAttribute(XMLBuilder._Static.A_ORDER); %>
...
for (int i= 0; i != numgroups; i++)
  ...
  int remainingsubs= Integer.parseInt(group.getAttribute(XMLBuilder._Static.A_REMAININGSUBS));
  NodeList members= group.getElementsByTagName(XMLBuilder._Static.TAG_MEMBER); 
  int nummembers= members.getLength();
  String lateTag = group.hasAttribute(XMLBuilder._Static.A_LATESUBMISSION) ? " style=\"background-color: rgb(218, 186, 186);\"" : ""; 
  /* 0 for the total score */
  Element grade= (Element)group.getElementsByTagNameNS(XMLBuilder._Static.TAG_GRADE + "0", XMLBuilder._Static.TAG_GRADE).item(0); 
  Element regrade= (Element)group.getElementsByTagName(XMLBuilder._Static.TAG_REGRADE).item(0);
  String status= regrade != null ? regrade.getAttribute(XMLBuilder._Static.A_STATUS) : ""; 
  boolean overMax= (grade==null ? false : grade.hasAttribute(XMLBuilder._Static.A_OVERMAX));
  boolean isAveraged = (grade == null) ? false : grade.hasAttribute(XMLBuilder._Static.A_ISAVERAGE);
  boolean hasExtension=group.hasAttribute(XMLBuilder._Static.A_EXTENSION);
  boolean canGrade = isAdmin || (isGrades && (!assignedGraders || group.hasAttribute(XMLBuilder._Static.A_CANGRADE)));
   %>
  <tr <%
  if (status.equals(RegradeRequest._Static.PENDING)) { %> class="pending"<%
  ...
  for (int j= 0; j != members.getLength(); j++) {
    ...
    String startTag = null, endTag = null;
     String gradeURL = "?" + AccessController.P_ACTION + "=" + AccessController.ACT_STUDENTALLGRADES + "&" 
          + AccessController.P_COURSEID + "=" + courseid + "&" + AccessController.P_NETID + "=" + member.getAttribute(XMLBuilder._Static.A_NETID);
    if (isAdmin) {
...
%>
    <%= startTag + member.getAttribute(XMLBuilder._Static.A_NETID) + endTag %>
...
<%  } %>
 ...
 <%--                          GROUPS                    --%>     
  ...
  for (int j= 0; j != members.getLength(); j++) {
    Element member= (Element)members.item(j);
    if (member.hasAttribute(XMLBuilder._Static.A_SECTION)) {
    	section = member.getAttribute(XMLBuilder._Static.A_SECTION);
    	if ((section == "") || (section == null)) section = "&nbsp;";
    }
%>
   <%=  member.getAttribute(XMLBuilder._Static.A_LASTNAME) + ", " + member.getAttribute(XMLBuilder._Static.A_FIRSTNAME)+" "%>
    <% if (j!=members.getLength()-1) { %> <br> <% } %>
...
<% if (isAdmin || isGrades) { %>
  <% String hiddenAppend;
     if (hasExtension) {
        hiddenAppend= "<div style=\"display: none;\">"+group.getAttribute(XMLBuilder._Static.A_EXTVAL)+"</div>";
    } else {
 ...
 <%--                          EXTENSION                    --%>
    ...
    <%
    String extAMPM = group.hasAttribute(XMLBuilder._Static.A_EXTAMPM) ? group.getAttribute(XMLBuilder._Static.A_EXTAMPM) : assign.getAttribute(XMLBuilder._Static.A_DUEAMPM); 
    String extTime = group.hasAttribute(XMLBuilder._Static.A_EXTTIME) ? group.getAttribute(XMLBuilder._Static.A_EXTTIME) : assign.getAttribute(XMLBuilder._Static.A_DUETIME);
	if (hasExtension) {%>
	  <div style="background-color: #FFFFCC; text-align: center">
         <b>Extended Until:</b><br>
         <%=group.getAttribute(XMLBuilder._Static.A_EXTENSION)%>, <%=extTime%> <%=extAMPM%>&nbsp;<%
    } else { %>
      <div><%
    } %>
		<font size="1">  
		...
		<div class="showhide" id="date_<%=groupid%>" style="display:none">
          <div class="dateblock" >
            ...
            <input id="datebox_<%=groupid%>" type="text" size="17" name="<%= AccessController.P_DUEDATE + groupid%>" value="<%= hasExtension ? group.getAttribute(XMLBuilder._Static.A_EXTENSION) : assign.getAttribute(XMLBuilder._Static.A_DUEDATE) %>">
            ...
            <span class="example">e.g. <%= root.getAttribute(XMLBuilder._Static.A_EXAMPLEDATE) %></span><br>
...
<%
  String assignee="";
  if (numprobs == 0) {
    Element assignedto= (Element)group.getElementsByTagNameNS(XMLBuilder._Static.TAG_ASSIGNEDTO + "0", XMLBuilder._Static.TAG_ASSIGNEDTO).item(0); 
    if (assignedto != null) { 
        assignee= assignedto.getAttribute(XMLBuilder._Static.A_FIRSTNAME)+" "+assignedto.getAttribute(XMLBuilder._Static.A_LASTNAME);
    }
  %>
	...
	<% if (assignedto == null) { %>
        ...
        <% } else { %>
            ...
            <%=assignedto.getAttribute(XMLBuilder._Static.A_NETID) %></div>  
        <% }%>
</td><%
  }
  for (int j= 0; j != numprobs; j++) { 
    Element prob= (Element)probs.item(j); 
    String id= prob.getAttribute(XMLBuilder._Static.A_SUBPROBID); 
    Element assignedto= (Element)group.getElementsByTagNameNS(XMLBuilder._Static.TAG_ASSIGNEDTO + id, XMLBuilder._Static.TAG_ASSIGNEDTO).item(0); 
  if (assignedto!=null)
    assignee= assignedto.getAttribute(XMLBuilder._Static.A_FIRSTNAME)+" "+assignedto.getAttribute(XMLBuilder._Static.A_LASTNAME);
		    %>
    <td nowrap align="center" style="display: none" id="assigned_<%= assignedCount++ %>" name="assigned"<%= lateTag %>>
...
<%	boolean overMaxProb = false;
	for(int j = 0; j < numprobs; j++) {
		Element prob= (Element)probs.item(j);
		Element probGrade= (Element)group.getElementsByTagNameNS(XMLBuilder._Static.TAG_GRADE + prob.getAttribute(XMLBuilder._Static.A_SUBPROBID), XMLBuilder._Static.TAG_GRADE).item(0);
		overMaxProb = (probGrade==null ? false : probGrade.hasAttribute(XMLBuilder._Static.A_OVERMAX));%>
	<td nowrap align="right" class="subprob_score_cell"<%= group.hasAttribute(XMLBuilder._Static.A_LATESUBMISSION) ? " style=\"background-color: rgb(218, 186, 186)\"" : "" %>">
<%		if(overMaxProb) {%>
		<img src="images/warning.gif" height="15px" alt="(Over Max Score)">
<%		}%>
		<%= probGrade == null ? " " : probGrade.getAttribute(XMLBuilder._Static.A_SCORE) %>
	</td>
<%	}%>
...
<%	if(status.equals(RegradeRequest._Static.PENDING)) {%>
      <img src="images/tag_red.gif" alt="(Regrade pending)"><%
	} else if(status.equals(RegradeRequest._Static.REGRADED)) {%>
      <img src="images/tag_orange.gif" alt="(Regraded)"><%
	}
	if(overMax) {%>
     <img src="images/warning.gif" height="15px" alt="(Over Max)">
<%	} %>
		<%= (grade == null) ? "&nbsp;" : grade.getAttribute(XMLBuilder._Static.A_SCORE) %><%
  	if(isAveraged) {%>
		<b>~</b>
<%	}%>
    </td>

<%@ page language="java" import="org.w3c.dom.*, edu.cornell.csuglab.cms.www.*, edu.cornell.csuglab.cms.www.xml.*, java.util.*, edu.cornell.csuglab.cms.base.RegradeRequestBean, edu.cornell.csuglab.cms.author.Principal,edu.cornell.csuglab.cms.util.*,edu.cornell.csuglab.cms.base.*" %><%
/**********************************************************
...
Element assigns = (Element)course.getElementsByTagName(XMLBuilder.TAG_ASSIGNMENTS).item(0);
Element assignment= (Element)XMLUtil.getChildrenByTagNameAndAttributeValue(assigns, XMLBuilder.TAG_ASSIGNMENT, XMLBuilder.A_ASSIGNID, asgnID).item(0);
NodeList groups= root.getElementsByTagName(XMLBuilder.TAG_GROUP);
int numgroups= groups.getLength();
NodeList submissions = root.getElementsByTagName(XMLBuilder.TAG_SUBMISSION);
...
Element totalprob= XMLUtil.getFirstChildByTagName(root, XMLBuilder.TAG_TOTALPROBLEM);
...
for (Iterator j= probs.iterator(); j.hasNext();) {
	Element prob= (Element)j.next();
    String probID= prob.getAttribute(XMLBuilder.A_SUBPROBID); 
    String probName = prob.getAttribute(XMLBuilder.A_NAME); %>
...
<jsp:include page="../navbar.jsp"/>
    <td id="course_page_container">
      <div id="course_page">
        <span class="assignment_title"><%= assignment.getAttribute(XMLBuilder.A_NAME) %>- Grade Students</span>
        <br><br>
				<jsp:include page="../../problem-report.jsp"/>
        <form action="?<%= AccessController.P_ACTION %>=<%= AccessController.ACT_SETSTUDENTGRADES %>&amp;<%= AccessController.P_ASSIGNID %>=<%= assignment.getAttribute(XMLBuilder.A_ASSIGNID) %>" method="post" enctype="multipart/form-data">
<%
    ...
    Element member= (Element)members.item(j); %>
              <%= j == 0 ? "" : "," %>
               <%= member.getAttribute(XMLBuilder.A_FIRSTNAME) %> <%= member.getAttribute(XMLBuilder.A_LASTNAME) %>
                  ...
                  (
			              <% if (isAdmin) { %>
			                <a style="color:white;" href="?<%= AccessController.P_ACTION %>=<%= AccessController.ACT_STUDENTALLGRADES %>&amp;<%= AccessController.P_COURSEID %>=<%= courseID %>&amp;<%= AccessController.P_NETID %>=<%= member.getAttribute(XMLBuilder.A_NETID) %>">
			              <% } %>
...
<% } %>
  ...
  <%if (nummembers != 1) { %>
    ...
    String netID= member.getAttribute(XMLBuilder.A_NETID);
...
<%
	if(isQuiz)
	{
		...
		for (Iterator k= probs.iterator(); k.hasNext();) {
			...
			String probid= prob.getAttribute(XMLBuilder.A_SUBPROBID);
			Element answer = (Element)member.getElementsByTagNameNS(XMLBuilder.TAG_ANSWERS + probid, XMLBuilder.TAG_ANSWERS).item(0); 
			...
			if(answer != null)
			{
				answerCount++;
				%>
					<%=problemPrefix + answer.getAttribute(XMLBuilder.A_ANSWERTEXT)%>
				<br>
				<%
			}	
		}
...
<%---------------------------------------------------------------------------------------------------------%>
      ...
      if (numprobs == 0 && hasTotal) { 
         ...
         boolean canGrade = !checkCanGrade || (score != null && score.hasAttribute(XMLBuilder.A_CANGRADE));
              %>
              ...
              <ul><li><input id="totalscore_<%= netID + "_" + groupid %>" type="text" onkeyup="onKeyupHandlerNoProbs('<%= netID %>','<%= groupid %>','0');" size="3" name="<%= AccessController.P_GRADE %>_<%= netID %>_0_<%= groupid %>" <%= score != null ? "value=\"" + score.getAttribute(XMLBuilder.A_SCORE) + "\"" : "value=\"\"" %><%= canGrade ? "" : " disabled" %>> /<%= totalprob.getAttribute(XMLBuilder.A_SCORE) %></li></ul>
            <script type="text/javascript">
...
<%
        ...
        if (numprobs == 1) {
          ...
          String problemOrder = prob.getAttribute(XMLBuilder.A_ORDER);
          String problemName = (problemOrder.equals("0")) ? prob.getAttribute(XMLBuilder.A_NAME) : problemOrder + ". " + prob.getAttribute(XMLBuilder.A_NAME);
          ...
          %>
			...
			<input type="hidden" name="<%= AccessController.P_OLDGRADE + "_" + netID + "_" + probid + "_" + groupid %>" value="<%= score != null ? score.getAttribute(XMLBuilder.A_SCORE) : "" %>">
              <ul>
              	...
              	<% if (prob.hasAttribute(XMLBuilder.A_CORRECTANSWER)) {%>
					<strong>Correct answer:</strong> 
					<%= prob.getAttribute(XMLBuilder.A_CORRECTANSWER) %><br />
				<% } %>
...
<%
          ...
          <% if (hasTotal) { %>
              <li><strong>Total:</strong>&nbsp;&nbsp;&nbsp;<span id="totalscore_<%= netID + "_" + groupid %>">&nbsp;<%= totalgrade == null ? "&nbsp;" : totalgrade.getAttribute(XMLBuilder.A_SCORE) %></span> /<%= totalprob.getAttribute(XMLBuilder.A_SCORE) %></li>
          <% } %>
              </ul>
<%        } 
      }%>
        ...
        pendingregrades++; 
            if (canRegrade) {
             ...
             <%     if (regrade.getAttribute(XMLBuilder.A_SUBPROBID).equals("0")) {
              %>
                    <strong>Respond</strong> to request for regrade on <strong><%=assignment.getAttribute(XMLBuilder.A_NAME) %></strong>
             <%  } else { 
              %>     
...
<%
      ...
      if (numprobs == 0) {
        ...
        if (numprobs == 1) {
          ...
          boolean canGrade = !checkCanGrade || (score != null && score.hasAttribute(XMLBuilder.A_CANGRADE)); %>
			<input type="hidden" name="<%= AccessController.P_OLDGRADE + "_" + netID + "_" + probid + "_" + groupid %>" value="<%= score != null ? score.getAttribute(XMLBuilder.A_SCORE) : "" %>">
              <ul><li><%= prob.getAttribute(XMLBuilder.A_NAME) %>:&nbsp;&nbsp;&nbsp;<input id="score_<%= netID + "_" + probid + "_" + groupid %>" type="text" onkeyup="onKeyupHandlerNoProbs('<%= netID %>','<%= groupid %>','<%= probid %>');" size="3" name="<%= AccessController.P_GRADE %>_<%= netID %>_<%= probid %>_<%= groupid %>" <%= score != null ? "value=\"" + score.getAttribute(XMLBuilder.A_SCORE) + "\"": "" %><%= canGrade ? "" : " disabled" %>> /<%= prob.getAttribute(XMLBuilder.A_SCORE) %></li></ul>
        	<script type="text/javascript">
             // Place above input in table
             scores['<%= groupid %>']['<%= probid %>']['<%= netID %>']=  getElementById('score_<%= netID + "_" + probid  + "_" + groupid %>');
			</script>
       ...
       <% } else { %>
          ...
          for (Iterator k= probs.iterator(); k.hasNext();) {
            ...
            boolean canGrade = !checkCanGrade || (score != null && score.hasAttribute(XMLBuilder.A_CANGRADE)); 
                if (score != null) {%>
				<input type="hidden" name="<%= AccessController.P_OLDGRADE + "_" + netID + "_" + probid + "_" + groupid %>" value="<%= score != null ? score.getAttribute(XMLBuilder.A_SCORE) : "" %>">
               		<li><%= prob.getAttribute(XMLBuilder.A_NAME) %>:&nbsp;&nbsp;&nbsp;<input <%= hasTotal ? "onkeyup=\"onkeyupHandler('" + netID + "', '" + probid + "', '" + groupid + "')\"" : "" %> id="score_<%= netID + "_" + probid  + "_" + groupid %>" type="text" size="3" name="<%= AccessController.P_GRADE %>_<%= netID %>_<%= probid %>_<%= groupid %>" <%= score != null ? "value=\"" + score.getAttribute(XMLBuilder.A_SCORE) + "\"" : "" %><%= canGrade ? "" : " disabled" %>> /<%= prob.getAttribute(XMLBuilder.A_SCORE) %></li>
                <script type="text/javascript">
                  // Place above input in table
                  scores['<%= groupid %>']['<%= probid %>']['<%= netID %>']=  getElementById('score_<%= netID + "_" + probid  + "_" + groupid %>');
								</script><%
            ...
            <% if (hasTotal) { %>
              <li>Total:&nbsp;&nbsp;&nbsp;<span id="totalscore_<%= netID + "_" + groupid %>">&nbsp;<%= totalgrade == null ? "&nbsp;" : totalgrade.getAttribute(XMLBuilder.A_SCORE) %></span> /<%= totalprob.getAttribute(XMLBuilder.A_SCORE) %></li>
            <% } %>
              </ul>
<%      } 
      }%>      
...
<% if (numregrades != 0) { %>
  ...
  for (int j= 0; j != numregrades; j++) { 
    ...
    if (regraderesponse != null) {
      ...
      boolean canRemove = !checkCanGrade || meNetID.equalsIgnoreCase(regraderesponse.getAttribute(XMLBuilder.A_NETID)); %>
                <dd>
...
<%
    ...
    for (int j= 0; j != numcomments; j++) { 
        ...
        boolean canRemove = !checkCanGrade || meNetID.equalsIgnoreCase(comment.getAttribute(XMLBuilder.A_NETID));
%>
  <div class="<%= j % 2 == 0 ? "row_even" : "row_odd"%>">

        <h2><strong>Comment:</strong> by <%=comment.getAttribute(XMLBuilder.A_NETID)%> on <%=comment.getAttribute(XMLBuilder.A_DATE)%>
        <% if (canRemove) { %>
        <span style="float: right"><input type="checkbox" name="<%= AccessController.P_REMOVECOMMENT + comment.getAttribute(XMLBuilder.A_COMMENTID) %>"> Remove Comment</span>
        <% } %>
        </h2>

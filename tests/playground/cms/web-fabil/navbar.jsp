<%@ page language="java" import="org.w3c.dom.*, cms.www.*, cms.auth.*, cms.model.*, cms.www.xml.*" %><%
 ...
 boolean isStudent = course != null ? course.hasAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ISSTUDENT()) : false;
 String courseid = (course != null ? course.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_COURSEID()) : null);
 String URL = request.getServletPath();
 Principal p = (Principal)session.getAttribute(AccessController.A_PRINCIPAL);
 NodeList allassigns = new CMSNodeList();
 try {
 	Element assigns = (Element) root.getElementsByTagName(XMLBuilder._Static._Proxy.$instance.get$TAG_ASSIGNMENTS()).item(0);
 	if (assigns != null) {
...
if (courseid != null) {%>
  ...
  <a <%= (allassigns.getLength() == 0) ? "class=\"unavailable\"" : "" %> href="#">Assignments</a>
	...
	else for (int i = 0; i < allassigns.getLength(); i++)
	{
		...
		if (xAssign.getNodeName().equals(XMLBuilder._Static._Proxy.$instance.get$A_ASSIGNMENT())) {
			String status = xAssign.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_STATUS());
		    boolean isHidden = status.equals(Assignment._Static._Proxy.$instance.get$HIDDEN());
			int type = Integer.parseInt(xAssign.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ASSIGNTYPE()));
			String action = "";
			
			if (type == Assignment._Static._Proxy.$instance.get$ASSIGNMENT())
				action = AccessController.ACT_ASSIGN;
			else if (type == Assignment._Static._Proxy.$instance.get$SURVEY())
				action = AccessController.ACT_SURVEY;
			else if (type == Assignment._Static._Proxy.$instance.get$QUIZ())
				action = AccessController.ACT_QUIZ;
			
			isHidden = status.equals(Assignment._Static._Proxy.$instance.get$HIDDEN());
			if (!isHidden) {%>
	<li><a href="?<%= AccessController.P_ACTION%>=<%=AccessController.ACT_ASSIGN%>&amp;<%=AccessController.P_ASSIGNID%>=<%=xAssign.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ASSIGNID())%>">

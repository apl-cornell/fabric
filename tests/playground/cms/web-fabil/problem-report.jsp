...
*
* The correct format for including a status report is that used by XMLBuilder._Static._Proxy.$instance.get$()addStatus().
*
...
String error = root.hasAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ERR_NAME()) ? root.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ERR_NAME()) : null;
...
<%			}%>
        <span class="<%= status.hasAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ISERROR()) ? "status_err" : (status.hasAttribute(XMLBuilder._Static._Proxy.$instance.get$A_ISWARNING()) ? "status_warn" : "status_ok") %>" align="center">
        	<%= status.getAttribute(XMLBuilder._Static._Proxy.$instance.get$A_TEXT()) %>
        </span>
<%	 	}
	}%>
<%	if (error != null) { %>
